...
#ifndef GRID_TENSOR_METAL_GENERATOR_H
#define GRID_TENSOR_METAL_GENERATOR_H
...
class GeneratorOperator<TOperator<device::Metal>>
{
                       ...
                       F gen) const
  {
    for (size_t i = 0; i < dimensions[0]; i++, dst += strides[0])
      TOperator<device::Metal>()(dst, gen);
  }
...
template <> struct FillOperator<device::Metal>
{
  template <typename T> inline void operator()(T* dest, const T val) const { *dest = val; }
};

template <> struct FunctionOperator<device::Metal>
{
  // FIXME should it require: std::invocable<F&> && std::indirectly_writable<O, std::invoke_result_t<F&>>
...
#endif  // GRID_TENSOR_METAL_GENERATOR_H

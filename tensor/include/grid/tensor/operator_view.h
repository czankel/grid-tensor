...
// FIXME: also create "OperatorView"?
...
class TensorView
{
 public:
  ...
  // FIXME: adjust size with offset? why even provide size?? does size include offset???
  ...
  {
    // FIXME: use std::move or something? or is thi implicit?
  ...
  /// Size returns the data buffer size. FIXME offset??
                    ...
                    size_t offset = 0)
{
  ...
  size_t size = get_buffer_size<typename TTensor::value_type>(dimensions, strides);
...
#if 0
template <typename TTensor, size_t Rank>
inline auto Reshape(const TTensor& tensor, const std::array<size_t, Rank>& dimensions)
{
  using value_type = typename TTensor::value_type;
  auto strides = make_strides(dimensions);
  size_t size = get_bufer_size<value_type>(dimensions, strides);
  assert(size <= tensor.Size());
  return TensorView(tensor, dimensions, strides, size, 0);
}
#endif

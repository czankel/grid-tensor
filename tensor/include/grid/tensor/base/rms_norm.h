...
class TensorRmsNorm<Tensor, T, TRank, TTensor>
{
 ...
 public:
  ...
  requires (std::is_floating_point_v<value_type> && std::remove_cvref_t<TTensor>::rank > 0)
...
TensorRmsNorm(TTensor&&)
  -> TensorRmsNorm<Tensor, typename std::remove_cvref_t<TTensor>::value_type, std::remove_cvref_t<TTensor>::rank, TTensor&&>; //typename to_tensor<TTensor>::type>;

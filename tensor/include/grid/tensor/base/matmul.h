  ...
  void operator()(O&& o, R1&& r1, R2&& r2) const
  {
    ...
    else if constexpr (rank1 == 1 && rank2 == 1)
    {
      if (strides1[0] == 1 && strides2[0] == 1)
        ...
        //
 #if 0
  // default mat_T x vec multiplication with transposed matrix for non-contiguous matrix and vector.
  template <typename T>
  inline void MatTVec(T* dest, const T* src1, const T* src2,
                      std::span<const size_t, 2> dimensions,
                      const ssize_t& strides0,
                      std::span<const ssize_t, 2> strides1,
                      const ssize_t& strides2) const

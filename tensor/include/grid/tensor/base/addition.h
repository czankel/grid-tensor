...
class TensorAdd<Tensor, _Tp, _Rank, _Tensor1, _Tensor2>
{
 ...
 private:
  // GetFullDimensions expands "broadcast" dimension (dim = 1) to the dimension of the other tensor.
  std::array<size_t, _Rank> GetFullDimensions(const _Tensor1& tensor1, const _Tensor2& tensor2) const
  {
 ...
 public:
  ...
  auto operator()() const
  {
    auto dimensions = GetFullDimensions(tensor1_, tensor2_);
...
TensorAdd(_Tensor1, _Tensor2)
  -> TensorAdd<Tensor, typename _Tensor1::value_type, to_tensor<_Tensor1>::type::rank,

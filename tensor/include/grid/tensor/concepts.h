...
struct tensor_is_convertible_to :
  std::is_assignable<TTensor<typename TFrom::value_type, TFrom::rank, DynamicMemory<TDevice>>, TFrom> {};
#if 0
template <typename TFrom, template <typename, size_t, typename...> typename TTensor>
concept ConvertibleTo =
  std::is_class_v<std::remove_cvref_t<TFrom>> &&
  tensor_is_convertible_to<std::remove_cvref_t<TFrom>, TTensor>::value;
#endif
...
struct tensor_device<TTensor<T, TRank, DynamicMemory<TDevice>>> { using type = TDevice; };

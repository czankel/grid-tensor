...
namespace {
  ...
  template <> struct Eps<float>  { constexpr static float  value = 1e-5f; };
  template <> struct Eps<double> { constexpr static double value = 1e-5f; };
}

template <> class RmsNormOperator<device::Cuda>
{
  template <typename T> void Eval(T*, const T*, const T, size_t) const;
  template <typename T> void Eval(T*, const T*, const T, std::array<const size_t, 2>&& dimensions) const;
  template <typename T> void Eval(T*, const T*, const T*, const T, size_t) const;
  template <typename T> void Eval(T*, const T*, const T*, const T, std::array<const size_t, 2>&&) const;
 ...
 public:
  ...
  void operator()(I&& in, O&& out) const
  {
    ...
    if (n_rows == 1)
      Eval<value_type>(&*first_d, &*first_x, Eps<value_type>::value, row_size);
    else
      Eval<value_type>(&*first_d, &*first_x, Eps<value_type>::value, std::array<const size_t, 2>{n_rows, row_size});

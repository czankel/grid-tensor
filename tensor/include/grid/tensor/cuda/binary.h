...
#include "device.h"
#include "../binary.h" // FIXME??
#include "../concepts.h"  // FIXME??
...
class BinaryOperator<TOperator, device::Cuda>
{
  ...
  // non-contiguous
  template <typename T, size_t R>
  void EvalDiscontiguous(T*, const T*, const T*, std::span<const size_t, R>,
                         std::span<const ssize_t, R>, std::span<const ssize_t, R>, std::span<const ssize_t, R>) const;
 ...
 public:
  ...
  void operator()(I1&& in1, I2&& in2, O&& out) const
  {
    ...
    details::Fold([&](auto folded_dims, const bool contiguous) {
        ...
        constexpr static size_t s_x = strides_x.size();
        constexpr static size_t s_y = strides_y.size();
        if constexpr (s_x == 0 && s_y == 0)
        {
        ...
        else if constexpr (rank == 1)
        {
          ...
          else
          {
            ...
            EvalDiscontiguous(&*first_d, &*first_x, &*first_y, folded_dims,
                              std::span<const ssize_t, rank>(strides_d.begin(), rank),
                              std::span<const ssize_t, rank>(b_strides_x.begin(), rank),
                              std::span<const ssize_t, rank>(b_strides_y.begin(), rank));
           }
        }
        else if constexpr (rank < first_d.Rank())
        {
          ...
          EvalContiguous(&*first_d, &*first_x, &*first_y, folded_dims,
                         ...
                         std::span<const ssize_t, rank>(b_strides_x),
                         std::span<const ssize_t, rank>(b_strides_y));
        }
        else
        {
          auto [b_strides_x, b_strides_y] = BroadcastStrides(strides_x, strides_y);
          EvalDiscontiguous(&*first_d, &*first_x, &*first_y, folded_dims,
                            std::span<const ssize_t, rank>(strides_d),
                            std::span<const ssize_t, rank>(b_strides_x),
                            std::span<const ssize_t, rank>(b_strides_y));
         }

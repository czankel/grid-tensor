...
//
// FIXME: what about contiguous??? Even above, use a "loop" ??
...
{
  auto idx_x = metal::pos_to_index(pos, stride_x); // FIXME: change to PosToIndex??
                                    ...
                                    uint2 grid_dim [[threads_per_grid]])
{
  ...
  size_t idx_c = pos.x + (size_t)grid_dim.x * pos.y;
  d[idx_c] = Op()(x[idx_x], y[idx_y]);
}
                                    ...
                                    uint3 grid_dim [[threads_per_grid]])
{
  auto idx_x = metal::pos_to_index(pos, strides_x);
  auto idx_y = metal::pos_to_index(pos, strides_y);
  size_t idx_c = pos.x + (size_t)grid_dim.x * (pos.y + (size_t)grid_dim.y * pos.z);
  d[idx_c] = Op()(x[idx_x], y[idx_y]);
}

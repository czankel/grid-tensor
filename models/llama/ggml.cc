...
{
  "token_embd.weight",          // kEmbeddings      [vocab,dim]
  "blk.%d.attn_norm.weight",    // kAttentionNorm   [dim]
  ...
  "blk.%d.ffn_norm.weight",     // kFeedForwardNorm [dim]
  "output_norm.weight",         // kOutputNorm      [dim]
...
void GgmlFile::Load()
{
  ...
  for (uint64_t i = 0; i < n_kv_; ++i)
  {
    ...
    if (value.size == -1)
    {
      switch (value.type)
      {
        case kGgufTypeU8:       std::cout << std::any_cast<uint8_t>(value.value); break;
        case kGgufTypeI8:       std::cout << std::any_cast<int8_t>(value.value); break;
        case kGgufTypeU16:      std::cout << std::any_cast<uint16_t>(value.value); break;
        case kGgufTypeI16:      std::cout << std::any_cast<int16_t>(value.value); break;
        case kGgufTypeU32:      std::cout << std::any_cast<uint32_t>(value.value); break;
        case kGgufTypeI32:      std::cout << std::any_cast<int32_t>(value.value); break;
        case kGgufTypeU64:      std::cout << std::any_cast<uint64_t>(value.value); break;
        case kGgufTypeI64:      std::cout << std::any_cast<int64_t>(value.value); break;
        case kGgufTypeFloat32:  std::cout << std::any_cast<float>(value.value); break;
        case kGgufTypeFloat64:  std::cout << std::any_cast<double>(value.value); break;
        case kGgufTypeBool:     std::cout << std::any_cast<bool>(value.value); break;
        case kGgufTypeString:   std::cout << std::any_cast<std::string>(value.value); break;
        default: break;
      }
    }
    else
  ...
  for (uint64_t i = 0; i < n_tensors_; ++i)
  {
    ...
    // FIXME: debug
#if 0
    std::cout << "tensor " << name << " (" << type << ") [ " << rank << " ] ";
    for (int i = rank - 1; i > 0; i--)
      std::cout << dims[i] << ", ";
    std::cout << dims[0] << " offset " << offset << " size: " << size << '\n';
#endif
  }

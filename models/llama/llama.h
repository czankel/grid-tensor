...
class LLaMAModelT : public LLaMAModel
{
  ...
  //using Tensor1D = Tensor<T, N>;
  using Tensor1D = Tensor<T, 1, StdAllocator>;
  using Tensor2D = Tensor<T, 2, StdAllocator>;

  //using Tensor2D = Tensor<T, 2, grid::StdAlloc>;
 ...
 public:
  ...
  static LLaMAModelT* Load(LLaMAFile& file);
